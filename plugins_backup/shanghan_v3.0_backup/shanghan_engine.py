...
class ShanghanEngine:
    ...
    def analyze(self, user_symptoms):
        ...
        # Phase 1: Lock the Category (Meridian)
        category_scores = {}
        for r in sorted_initial_results:
            cat = r['category']
            if cat:
                category_scores[cat] = category_scores.get(cat, 0.0) + r['score']
        ...
        # Phase 2: Filter and keep only formulas from the locked category
        final_formulas = [r for r in sorted_initial_results if r['category'] == locked_category]
